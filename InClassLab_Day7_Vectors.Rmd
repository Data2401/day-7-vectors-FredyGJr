...
nums[7]
nums[5:8]
first_last <- c(nums[1:3], nums[(length(nums)-2):length(nums)])
...
```{r}
roll_two_dice <- function(){sum(sample(1:6, size = 2, replace = TRUE))}
roll_two_dice()
...
# Your one-roll function goes here
  roll_winner <- function(name1, name2){
    roll1 <- sample(1:6, 1)
    roll2 <- sample(1:6, 1)
    if (roll1 > roll2){
      return(name1)
      ...
      # The first tie thus reroll once
      roll1
      roll2
      if (roll1 > roll2){
        return(name1)
...
```{r}
roll_winner_15 <- function(player1, player2){
  # the simulated roll for the players 15 time
  p1 <- sample(1:6, size = 15, replace = TRUE)
  p2 <- sample(1:6, size = 15, replace = TRUE)
  ...
  #counts
  p1_count <- sum(p1_wins)
  p2_count <- sum(p2_wins)
  tie_count <- sum(ties)
  
  #report the counts
  print(paste(player1, "wins:", p1_count))
  print(paste(player2, "wins:", p2_count))
  print(paste("ties", tie_count))
...
roll_winner_15_weighted <- function(player1, player2){
  #player 1 will have a weighted dice
  p1 <- sample(1:6, size = 15, replace = TRUE,
               prob = c(0.01, 0.05, 0.10, 0.15, 0.20, 0.49))
...
roll_winner_15_weighted("Bob", "Daniel")
roll_winner_15_weighted("Bob", "Daniel")
roll_winner_15_weighted("Bob", "Daniel")
roll_winner_15_weighted("Bob", "Daniel")
roll_winner_15_weighted("Bob", "Daniel")
...
roll_winner_15_weighted("Bob", "Daniel")
roll_winner_15_weighted("Bob", "Daniel")
# Bob/player 1 always wins overall and out of 15 matches the lowest amount of wins he would get would be 8 and most of the time he at least won 10 times for each 15 rolls.
